...
impl LayoutEngine {
    ...
    fn get_cached_layout(
        ...
        if let Some(cached) = self.layout_cache.get(&node_id) {
            if cached.generation == generation
                && self.constraints_match(&cached.constraints, constraints)
            {
    ...
    pub fn get_layout_box(&self, node_id: NodeId) -> Option<LayoutBox> {
        self.layout_cache
            .get(&node_id)
            .map(|cache| cache.result.layout_box)
    }
    ...
    pub async fn layout_node_public(
        ...
        self.layout_node_recursive(node_id, constraints, document, style_engine, generation)
            .await
    }
}
